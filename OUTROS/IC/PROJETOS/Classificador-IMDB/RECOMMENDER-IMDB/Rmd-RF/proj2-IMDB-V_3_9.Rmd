...
```{r}
db2 = db1 %>% filter(WR <= mean(WR)) %>% count()
```
...
Observamos que, a partir do número de árvores geradas  $ntrees > 60$ o erro OOB (Out of Bag) não pode ser melhorado. 
...
__Modelo Final__
Os parâmetros utilizados, finalmente serão $m_{try} = 5$ e $n_{tree} = 60$
```{r}
set.seed(093180)
...
O Random Forest  apresenta overfiting para os dados de treinamento, como é possível observar na  matriz de confusão obtida, e a partir de 60 arvores de regressão no algoritmo, tem-se um ganho minimo ao adicionar novas árvores. 
