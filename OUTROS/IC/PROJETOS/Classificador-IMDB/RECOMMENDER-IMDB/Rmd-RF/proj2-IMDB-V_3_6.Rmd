...
list.of.packages <- c("arules","dplyr","HardyWeinberg","cdparcoord", "knitr", "knitLatex", "kableExtra", "tidyverse", "tidyverse", "cdparcoord", "cowplot", "ggpubr", "gridExtra", "stringi", "cluster", "factoextra", "cluster", "randomForest", "rpart.plot", "DT", "keras", "caret", "highcharter", "wordcloud", "e1071")
...
Semelhante à variável de gênero, foi feito o split por palavra chave da variável keyword e, posteriormente, geramos uma nuvem de palavras chave contidas na base de dados, com o auxílio do pacote `wordcloud2`. Esse, por sua vez continha 6780 palavras únicas. e foi gerado com o seguinte comando, após ter obter o data frame do nome das palavras chaves e frequência.
...
```{r}
library(VIM)
missing.values <- aggr(db, sortVars = T, prop = T, sortCombs = T, cex.lab = 1.5, cex.axis = .6, cex.numbers = 5, combined = F, gap = -.2)
```
...
```{r}
hc00 = hchart(db1$title_year, color = "#79d8a2", name = "imdb1_score") %>% 
...
- 2. Para cada amostra via bootstrap, it grows un-puned tree by escolha da melhor quebra da árvore baseado na amostra aleatória do valor predito de mtry a cada nó da árvore;  
...
No passo a seguir, iremos remover as variáveis  *Game_Show*, *Sci_Fi*, *Reality_TV*, *News*, *Short* e *Film_Noir* uma vez que estas não se mostraram importantes no algoritmo.
```{r}
train1 = train %>% select(-Sci_Fi, -Game_Show, -Reality_TV, -News, -Film_Noir, -Short)
test1 = test %>% select(-Sci_Fi, -Game_Show, -Reality_TV, -News, -Film_Noir, -Short)
```
