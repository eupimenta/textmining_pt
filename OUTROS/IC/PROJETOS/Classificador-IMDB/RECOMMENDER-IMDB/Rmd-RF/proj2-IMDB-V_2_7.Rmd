...
```{r}
hc1 = hchart(db1$imdb_score, color = "#e8bb0b", name = "imdb_score") %>% 
        hc_title(text = "Histograma dos votos na plataforma IMDB") %>%
        hc_exporting(enabled = TRUE, filename = "Fig1-Pimenta"); hc1
```

```{r}
hc2 = hchart(db1$num_voted_users, color = "#786eea", name = "imdb_num_votos") %>% 
...
```{r, echo = FALSE, eval = TRUE, message=FALSE, include = TRUE}
cat("Média, moda e mediana da variável imdb_score são, respectivamente,",round(mean(db1$imdb_score, na.rm = TRUE),2), Moda(db1$imdb_score),median(db1$imdb_score, na.rm = TRUE))
```
...
,imdb_score, movie_facebook_likes, plot_keywords)
glimpse(db1)
...
```{r}
cat("Foram inseridas todas as", dim(fe)[2],"novas variáveis provenientes dos gêneros sem duplicidade que foram descobertos na sessão anterior.")
```
...
Remove as variáveis *imdb_score* e *genres* e transformando todas as variáveis em numéricas.
```{r}
movie <- db2
