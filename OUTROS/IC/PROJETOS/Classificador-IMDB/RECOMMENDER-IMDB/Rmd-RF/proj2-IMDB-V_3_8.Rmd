...
list.of.packages <- c("arules","dplyr","HardyWeinberg","cdparcoord", "knitr", "knitLatex", "kableExtra", "tidyverse", "tidyverse", "cdparcoord", "cowplot", "ggpubr", "gridExtra", "stringi", "cluster", "factoextra", "cluster", "randomForest", "rpart.plot", "DT", "keras", "caret", "highcharter", "wordcloud", "e1071", "icon", "wordcloud2", "corrplot")
...
Decidimos por retirar todos os fatores que retornaram importância abaixo de 10. Portanto, ficaremos apenas com as seguintes variáveis:
```{r}
RF1 = RF1[1:13,]
hc6_1 <- highchart() %>%
  hc_add_series(data = RF1$importance, 
...
__Modelo Final__
Os parâmetros utilizados, finalmente serão $m_{try} = 3$ e $n_{tree} = 60$
```{r}
set.seed(093180)
...
```{r}
MDSplot(rf_final, train1$WR_Grp, pch=20)
```
...
A Random Forest  apresenta overfiting para os dados de treinamento, como é possível observar na  matriz de confusão obtida, e a partir de 100 arvores de regressão no algoritmo, tem-se um ganho minimo ao adicionar novas árvores. 

É possível comparar nossos resultados com os resultados da usuária Yueming (disponíveis no site Kaggle clicando [AQUI](https://www.kaggle.com/carolzhangdc/predict-imdb-score-with-data-mining-algorithms) e [AQUI](https://www.kaggle.com/carolzhangdc/analyze-imdb-score-with-data-mining-algorithms)), sendo que a partir de seus resultados é possível obter algumas sugestões de melhora do nosso algoritmo, principalmente quanto ao overfiting. O primeiro ponto, que não foi observado, foram linhas repetidas, onde ela observou 43 linhas repetidas e variáveis com excesso de respostas nulas. Quanto as observações repetidas ela as excluiu e quanto as respostas nulas em excesso, ela as tratou como variáveis NA. Ainda foram observadas variáveis que apresentavam categorias com muito poucas respostas, como por exemplo muitos poucos filmes antes de 1980. Tal situação foi enfrentada por ela excluindo observações muito raras, como filmes anteriores a 1980. De uma forma geral, a autora comparou três modelos (KNN, Árvore de Decisão e Random Forest) apenas levando em consideração a acurácia obtida nos modelos.

Aqui, tentamos levar em consideração os valores, principalmente, da matriz de confusão e não somente a acurácia. Percebemos que:

- O nosso modelo é muito bom para classificar um filme com (6,8]★, com uma sensibilidade de cerca de %. Entetanto, para as outras categorias o modelo se mostrou pobre e pouco robusto para o que se propôs: classificar. A exemplo disso, filmes na categoria com até 4★ tiveram um péssimo desempenho com uma sensibilidade de zero e especificidade 1, ou seja o modelo errou a classificação desse grupo. As categorias de (4,6]★ e (8,10]★ tiveram cerca de 34% e 50% de sensibilidade, respectivamente. Isso seu deu, possivelmente ao fato de as categorias serem muito desbalanceadas como podemos verificar abaixo.
...
Enquanto na base treino mais de 2mil filmes estavam na categoria de 6 a 8 estrelas, apenas 10 filmes se encontravam na categoria de até 4 estrelas. Esse cenário na base de teste foi propocionalmente semelhante, com 878 e 3 filmes nos gripos de 6 a 8 e 0 a 4 estrelas, respectivamente.

Ainda em análise de classificações corretas vs erradas através da matriz de confusão obtida com a predição dos dados da base de teste o modelo teve uma acurácia calculada de 83%. E, apesar do resultado da predição, conseguimos obter quais são os fatores/variáveis mais importantes para análise de classificação, dentre as variáveis existentes. Abaixo citamos as top 10 variáveis mais importantes.
