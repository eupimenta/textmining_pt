...
Semelhante à variável de gênero, foi feito o split por palavra chave da variável keyword e, posteriormente, geramos uma nuvem de palavras chave contidas na base de dados, com o auxílio do pacote `wordcloud2`. Esse, por sua vez continha 6780 palavras únicas.
...
No passo a seguir, removeremos as variáveis  *Game_Show*, *Sci_Fi*, *Reality_TV*, *News*, *Short* e *Film_Noir* uma vez que estas não se mostraram importantes no algoritmo.
```{r}
...
```{r}
RF1 = as_tibble(data.frame(variables = names(RF_importance1), importance = RF_importance1))
rownames(RF1) <- NULL
...
__Gráfico de dependência parcial__
```{r}
partialPlot(rf1, train1, num_voted_users, 'de 6 a 8')
```
