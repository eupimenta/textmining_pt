...
```{r}
cat("Foram inseridas todas as", dim(fe)[2],"novas variáveis provenientes dos gêneros únicos descobertos na sessão anterior.")
```
...
Remove as variáveis *imdb_score*, *genres*, *plot_keywords* e *movie_imdb_link* transformando todas as variáveis em numéricas.
```{r}
movie <- db1
...
```{r}
cat("Além disso, a nossa base de dados que iremos trabalhar no modelo tem", dim(movie)[1] , "observações e ", dim(movie)[2], "variáveis")
```
...
training <- movie[ind==1, -4]
test <- movie[ind==2, -4]
trainingMovie <- movie[ind==1, 4]
...
No passo a seguir, removeremos as variáveis  *color*, *director_name*, *language*, *country*, *content_rating* e *Film_Noir* uma vez que estas não se mostraram importantes no algoritmo.
```{r}
training1 = training %>% select(-Sci_Fi, -Game_Show, -Reality_TV, -News, -Short, -Film_Noir)
...
```{r}
rf1 <- randomForest(proximity = T,ntree = 64,do.trace = T,WR~.,data=training1)
rf1
...
```{r}
rp <- rpart::rpart(formula = WR~.,data=training)
rpart::plotcp(rp)
```
