...
-->
```{r}
p0.1 <- plotuni(telco, telco$Churn, "Churn") 
p0.1


plt <- ggplot(telco, aes(x=telco$Churn, fill=(Churn))) + 
        ggtitle("Churn") + 
        xlab("Churn") +
        ylab("Percentage")  +
        geom_bar(aes(y = 100*(..count..)/sum(..count..)), width = 0.7) + 
...
# Análise de Agrupamento{.tabset .tabset-fade .tabset-pills}

## Objetivo
Serão testados metódos de agrupamento para clientes de serviços de telefonia. Utilizaremos 3 medidas de agrupamentos: Euclidiana, Manhattan e Gower.
...
Além disso, para poder testar as distancias euclidianas e de manhattan necessito tranformar as variávei em numéricas.

```{r}
telco2<-telco[,-1]
for (i in 1:length(telco2[1,])) {
  telco2[,i]<-as.numeric(telco2[,i])
}
str(telco2)
```
...
fazendo histograma
```{r}
hist(telco2$MonthlyCharges)
hist(telco2$TotalCharges)
hist(telco2$tenure)
```
plotando
```{r}
plot(telco2$TotalCharges,telco2$MonthlyCharges)
plot(telco2$TotalCharges,telco2$ternure)
plot(telco2$MonthlyCharges,telco2$ternure)
```
