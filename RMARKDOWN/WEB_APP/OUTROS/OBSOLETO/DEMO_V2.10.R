...
# Input de dados em formato excel apenas com duas colunas onde coluna1 = 'Protocolo' e coluna2 = 'DESCRI_PEDIDO'
# Algumas visualizações prévias da tabela inputada (.xlsx) como: str(); summary() e DT()
# Conversão da tabela original de textos em data.frame contendo apenas Protocolo e variáveis inseridas no modelo (0 e 1)
# Modelo Random Rorest Implementado pronto para uso
  ...
  list(
    ...
    server = function(input,output,session) {
      ...
      output$dev <- renderDataTable({
        ...
        textmining = function(PEDIDO){
          
          ## LETRAS MINUSCULAS
          PEDIDO = as.character(tolower(PEDIDO))
          ...
          PEDIDO = gsub("[:?:]","",PEDIDO)
          PEDIDO = gsub("[:-:]","",PEDIDO)
          PEDIDO = gsub("[:__:]","",PEDIDO)
          PEDIDO = gsub("[:;:]","",PEDIDO)
          PEDIDO = gsub("[:/:]","",PEDIDO)
          PEDIDO = gsub("[:(:]","",PEDIDO)
          PEDIDO = gsub("[:):]","",PEDIDO)
          PEDIDO = gsub("[:%:]","",PEDIDO)
          PEDIDO = gsub("[:º:]","",PEDIDO)
          PEDIDO = gsub("[:°:]","",PEDIDO)
          PEDIDO = gsub("[:ª:]","",PEDIDO)
          PEDIDO = gsub("\\d+","",PEDIDO)
          PEDIDO = gsub("[0-9]","",PEDIDO)
          PEDIDO = gsub("[:\n\t:]"," ",PEDIDO)
          PEDIDO = gsub("[:\t:]","",PEDIDO)
          PEDIDO = gsub("[:\n:]","",PEDIDO)
          PEDIDO = gsub("\\s+"," ",PEDIDO)
          PEDIDO = gsub("\"","",PEDIDO)
          
          ## STEMMINGS
          PEDIDO = ptstem(rslp(PEDIDO), complete = TRUE)
          PEDIDO = gsub("\\s+"," ",PEDIDO)
          
          ## STOP WORDS
          mystopwords <- data_frame(palavra = stopwords_pt)[,1]
          for (j in 1:length(PEDIDO)) {
            for(i in 1:dim(mystopwords)[1]){
              stopw = as.character(mystopwords[i,1])
              PEDIDO[j] = gsub(paste0("\\ ",stopw," "), " ", as.character(PEDIDO[j]))
            }
          }
        ...
        # Bag of Words (matriz de zeros e uns)        
        (col_input = ncol(DB()))
        (lin_input = nrow(DB()))
        ...
        for(i in 1:lin_input){
          for(j in 1:length(var_modelo)){
            g <- grepl(var_modelo[j], rawData()[[2]][i])
            if(g == TRUE){
              fe[i, j] <- 1        
            }
          }
        ...
        as_tibble(cbind(Protocolo = rawData()[[1]],fe)) %>% head()
