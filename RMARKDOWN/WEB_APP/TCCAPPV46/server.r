...
txt2 <- paste0("O dataset de histórico de pedidos possui dimensões de ", dim(db_modelo)[1]," linhas (observações/pedidos) por ", dim(db_modelo)[2], " colunas/variáveis. Donde a i-ésima linha da primeira coluna diz respeito ao dado da categoria cujo pedido foi classificado: DEA, DEE e OUTROS, e todas as demais k=", dim(db_modelo)[2]-1, " colunas dizem respeito a variáveis 0 ou 1 (binárias) indicando a ocorrência ou não do k-ésimo termo (palavra-chave/token) no i-ésimo pedido registrado, para todo i=1,2,...,", dim(db_modelo)[2][-1],".")
...
shinyServer(function(input,output,session) {
  ...
  output$tb <- renderUI({
    if(is.null(input$file)) {return(
      mainPanel(
        ...
        list(
          p("Esse aplicativo web (webapp) é componente o trabalho de conclusão de curso de Alize de Fárima Antunes Lea e Ewerson Carneiro Pimenta apresentado à Escola Nacional de Ciências Estatísticas do Instituto Brasileiro de Geografia e Estatística como requisito parcial à obtenção do título de Bacharel em Estatística."),
          br(),
          p("Essa aplicação tem por finalidade simular uma demonstração de utilidade de um modelo de classificação de texto via Random Forest (RF). Donde o problema inicial proposto é derivado de uma classificação multinomial com 3 categorias principais: "),
          list(p("1: DEA"),
               p("2: DEE"),
               p("3: OUTROS.")),
          img(src = "https://uploads.toptal.io/blog/post_image/120783/nlp-tutorial-text-classification-60b609c0b7a2622d2b0d6122f2b27f97.png", width = 550),
          br(),
  ...
  output$downloadData <- downloadHandler(
    ...
    filename = function(){
      ...
      paste("DadosClassificados-", as.character(format.Date(Sys.time(), "RJ%Y%m%d%-%Hh%M")), ".csv", sep="")
    },
