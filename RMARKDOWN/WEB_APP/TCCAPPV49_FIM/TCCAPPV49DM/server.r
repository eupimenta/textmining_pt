...
#getwd(),
#setwd("/Users/ewersonpimenta/Desktop/ESIC_TCC/TCC_v2.1/RMARKDOWN/WEB_APP/"),
#db_modeloz <- read.csv("db_modelo_rf_v10.csv")
db_modelo <- read.csv("DB_LAI-EPE_HISTORICO.csv") # dim(db_modelo); dim(fe)
db_modelo <- db_modelo[,-(1:3)]
#dim(db_modeloz); dim(db_modelo)
#head(colnames(db_modeloz)); head(colnames(db_modelo))
#tail(colnames(db_modeloz)); tail(colnames(db_modelo))

db_modelo0 <- read.csv("DB_LAI-EPE_HISTORICO.csv") # dim(db_modelo); dim(fe)
